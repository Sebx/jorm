...
impl ExecutionResult {
    ...
    fn update_metrics(&mut self) {
        ...
        let durations: Vec<Duration> = self.task_results.values().map(|r| r.duration).collect();
        ...
        // Sum total retries
        self.metrics.total_retries = self.task_results.values().map(|r| r.retry_count).sum();
    }
