...
use crate::executor::{
    ExecutionContext, TaskResult, ExecutorError, Task, TaskConfig, TaskStatus, AuthConfig
};
    ...
    async fn process_http_response(
        ...
        match execution_result {
            Ok(Ok(response)) => {
                ...
                println!("📡 HTTP response: {} {}", status_code, response.status().canonical_reason().unwrap_or(""));
                ...
                let output_data = if response_body.trim().starts_with('{') || response_body.trim().starts_with('[') {
                ...
                metadata.insert("content_length".to_string(), serde_json::json!(response_body.len()));
                ...
                metadata.insert("response_headers".to_string(), serde_json::json!(response_headers));
                ...
                Ok(TaskResult {
                    ...
                    error_message: if status == TaskStatus::Failed {
                        Some(format!("HTTP request failed with status code: {}", status_code))
                    } else {
                        None
                    },
                    metadata,
...
impl crate::executor::TaskExecutor for HttpTaskExecutor {
    async fn execute(&self, task: &Task, context: &ExecutionContext) -> Result<TaskResult, ExecutorError> {
        ...
        match &task.config {
            TaskConfig::HttpRequest { method, url, headers, body, auth, timeout } => {
    ...
    fn validate_task(&self, task: &Task) -> Result<(), ExecutorError> {
        match &task.config {
            TaskConfig::HttpRequest { method, url, headers: _, body: _, auth: _, timeout: _ } => {
    ...
    async fn test_http_get_request() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                if error_msg.contains("network") || error_msg.contains("dns") || error_msg.contains("connection") {
