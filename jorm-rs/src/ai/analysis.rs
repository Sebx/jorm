...
use crate::ai::{DAGAnalysis, OptimizationSuggestion, PotentialIssue, ResourceUsageEstimate, ComplexityMetrics, OptimizationType, ImpactLevel, EffortLevel, IssueType, Severity};
...
impl DAGAnalysisEngine {
    ...
    pub async fn analyze(&self, dag: &Dag) -> Result<DAGAnalysis> {
        ...
        let optimization_suggestions = self.generate_optimization_suggestions(dag, &complexity_metrics).await?;
    ...
    fn calculate_max_depth(&self, dag: &Dag) -> usize {
        ...
        let mut max_depth = 0;
        let mut queue: VecDeque<(String, usize)> = root_nodes.into_iter()
            .map(|task| (task, 0))
            .collect();
    ...
    async fn generate_optimization_suggestions(
        ...
        if self.has_parallelization_opportunities(dag) {
            suggestions.push(OptimizationSuggestion {
                ...
                description: "Some tasks can be executed in parallel to improve performance".to_string(),
        ...
        if dag.tasks.len() > 10 {
            suggestions.push(OptimizationSuggestion {
                ...
                description: "Consider breaking down large DAGs into smaller, more manageable pieces".to_string(),
                impact: ImpactLevel::Medium,
                implementation_effort: EffortLevel::High,
                suggested_changes: None,
            });
        ...
        suggestions.push(OptimizationSuggestion {
            ...
            description: "Consider adding retry mechanisms and error handling for critical tasks".to_string(),
            impact: ImpactLevel::High,
    ...
    fn identify_parallel_tasks(&self, dag: &Dag) -> Vec<Vec<String>> {
        ...
        while !remaining_tasks.is_empty() {
            ...
            let current_level: Vec<String> = remaining_tasks.iter()
    ...
    fn identify_potential_issues(&self, dag: &Dag, metrics: &ComplexityMetrics) -> Vec<PotentialIssue> {
        ...
        if metrics.cyclomatic_complexity > 15 {
            issues.push(PotentialIssue {
                ...
                description: "High cyclomatic complexity may impact performance and maintainability".to_string(),
                severity: Severity::Warning,
                affected_tasks: dag.tasks.keys().cloned().collect(),
                suggested_fix: Some("Consider breaking down complex tasks into smaller, simpler ones".to_string()),
            });
        }
        ...
        if metrics.max_depth > 8 {
            issues.push(PotentialIssue {
                issue_type: IssueType::PerformanceIssue,
                description: "Deep dependency chains may cause delays and make debugging difficult".to_string(),
                severity: Severity::Warning,
                affected_tasks: dag.tasks.keys().cloned().collect(),
        ...
        for (task, count) in task_dependency_count {
            if count > 5 {
                issues.push(PotentialIssue {
                    ...
                    description: format!("Task '{}' has many dependencies ({}), which may create bottlenecks", task, count),
                    severity: Severity::Warning,
                    affected_tasks: vec![task],
                    suggested_fix: Some("Consider breaking down this task or reducing its dependencies".to_string()),
                });
            }
        }
    ...
    fn estimate_resource_usage(&self, _dag: &Dag, metrics: &ComplexityMetrics) -> ResourceUsageEstimate {
...
mod tests {
    ...
    async fn test_complexity_metrics_calculation() {
        ...
        let engine = DAGAnalysisEngine::new(std::sync::Arc::new(crate::ai::RemoteAPIProvider::new().unwrap()));
