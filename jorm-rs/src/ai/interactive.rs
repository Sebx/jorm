...
impl InteractiveMode {
    ...
    fn show_welcome_message(&self) {
        ...
        println!("  • {} {}", "Natural Language:".bold(), "Ask questions in plain English".cyan());
        println!("  • {} {}", "Direct Commands:".bold(), "Use familiar CLI commands".cyan());
        println!("  • {} {}", "AI Intelligence:".bold(), "Smart suggestions and analysis".cyan());
        println!("  • {} {}", "Context Awareness:".bold(), "Remembers your workflow".cyan());
        println!();
    }
    ...
    fn show_quick_start(&self) {
        ...
        println!("  • {} {}", "Or:".bold(), "'Create a data processing pipeline'".cyan());
    ...
    async fn show_help(&self) -> Result<()> {
        ...
        println!("  • {} {}", "validate <file>".bold(), "- Validate DAG syntax".cyan());
        println!("  • {} {}", "describe <file>".bold(), "- Show DAG structure".cyan());
        println!("  • {} {}", "analyze <file>".bold(), "- AI-powered DAG analysis".cyan());
        println!("  • {} {}", "generate <description>".bold(), "- Create DAG from description".cyan());
        println!("  • {} {}", "schedule <file>".bold(), "- Schedule recurring execution".cyan());
        println!("  • {} {}", "daemon".bold(), "- Start scheduler daemon".cyan());
        println!("  • {} {}", "jobs".bold(), "- List scheduled jobs".cyan());
        println!("  • {} {}", "status".bold(), "- Show execution status".cyan());
        println!("  • {} {}", "list".bold(), "- List available DAGs".cyan());
        println!("  • {} {}", "model-info".bold(), "- Show AI model information".cyan());
        println!();
        ...
        println!("  • {} {}", "history".bold(), "- Show command history".cyan());
        println!("  • {} {}", "context <name>".bold(), "- Switch context".cyan());
        println!("  • {} {}", "pref <key> <value>".bold(), "- Set preference".cyan());
        println!("  • {} {}", "quit/exit".bold(), "- Exit interactive mode".cyan());
        println!();
        ...
        println!("  • Use {} to see available options", "Tab completion".italic());
        println!();
    ...
    async fn process_natural_language(&mut self, query: &str) -> Result<()> {
        ...
        match self.ai_service.model_provider.generate_response(query, &context).await {
            Ok(response) => {
    ...
    async fn update_preference(&mut self, key: &str, value: &str) -> Result<()> {
        match key {
            ...
            "suggestions" => {
                ...
                println!("✅ Auto-suggestions: {}", self.state.preferences.auto_suggestions);
            }
            _ => {
                println!("❌ Unknown preference: {}. Available: format, verbose, suggestions", key);
            }
        }
        Ok(())
    }
    ...
    async fn generate_dag(&self, description: &str) -> Result<()> {
        ...
        let dag = self.ai_service.generate_dag_from_natural_language(description).await?;
        ...
        println!("  🚀 Execute with: ./{} or jorm-rs run {}", filename, filename);
        Ok(())
    }
    ...
    async fn validate_generated_dag(&self, dag: &crate::parser::Dag, _filename: &str) -> Result<()> {
        ...
        for (task_name, task) in &dag.tasks {
            if let Some(script) = &task.config.data {
                if let Some(script_str) = script.as_str() {
                    ...
                    self.validate_python_script(script_str, task_name, &mut validation_errors, &mut validation_warnings);
                }
            }
        }
        ...
        if !validation_errors.is_empty() {
            println!("❌ Validation failed with {} error(s):", validation_errors.len());
    ...
    fn check_common_dag_issues(&self, dag: &crate::parser::Dag, warnings: &mut Vec<String>) {
        ...
        let referenced_tasks: std::collections::HashSet<String> = dag.dependencies
            .iter()
            .flat_map(|dep| vec![dep.task.clone(), dep.depends_on.clone()])
            .collect();
        
        for task_name in &task_names {
            if !referenced_tasks.contains(task_name) && dag.dependencies.iter().any(|d| d.task == *task_name) {
                warnings.push(format!("Task '{}' has no dependencies and is not referenced", task_name));
            }
        }
        ...
        for dep in &dag.dependencies {
            if !task_names.contains(&dep.task) {
                warnings.push(format!("Dependency references non-existent task: {}", dep.task));
            }
            if !task_names.contains(&dep.depends_on) {
                warnings.push(format!("Dependency references non-existent task: {}", dep.depends_on));
            }
        }
    }
    ...
    /// Basic Python script validation
    fn validate_python_script(&self, script: &str, task_name: &str, errors: &mut Vec<String>, warnings: &mut Vec<String>) {
        // Check for basic Python syntax issues
        if script.contains("import ") {
            // Check for common import issues
            if script.contains("import pandas") && !script.contains("import pandas as pd") {
                warnings.push(format!("Task '{}': Consider using 'import pandas as pd' for consistency", task_name));
            }
        }
        
        // Check for potential issues
        if script.contains("password") && script.contains("=") {
            warnings.push(format!("Task '{}': Hardcoded credentials detected - consider using environment variables", task_name));
        }
        ...
        if script.contains("pd.") && !script.contains("import pandas") {
            errors.push(format!("Task '{}': Uses pandas but doesn't import it", task_name));
        }
        
        if script.contains("pyodbc.") && !script.contains("import pyodbc") {
            errors.push(format!("Task '{}': Uses pyodbc but doesn't import it", task_name));
        }
        
        if script.contains("requests.") && !script.contains("import requests") {
            errors.push(format!("Task '{}': Uses requests but doesn't import it", task_name));
        }
        
        // Check for common Python issues
        if script.contains("print ") && !script.contains("print(") {
            warnings.push(format!("Task '{}': Consider using print() function instead of print statement", task_name));
        }
    }
    
    async fn schedule_dag(&self, file: &str, _cron: Option<&str>, _name: Option<&str>) -> Result<()> {
    ...
    async fn show_model_info(&self) -> Result<()> {
        ...
        println!("  Memory Usage: {} GB", model_info.memory_usage / 1_000_000_000);
...
mod tests {
    ...
    async fn test_command_parsing() {
        ...
        assert!(matches!(
            mode.parse_input("help"),
            InteractiveCommand::Help
        ));
        
        assert!(matches!(
            mode.parse_input("quit"),
            InteractiveCommand::Exit
        ));
    }
}
