...
use crate::ai::{ModelContext, ConversationTurn};
...
impl ChatInterface {
    ...
    async fn process_user_input(&self, input: &str) -> Result<()> {
        ...
        let context = ModelContext {
            ...
            system_prompt: Some("You are Jorm AI, a helpful assistant for DAG management and workflow automation.".to_string()),
            conversation_history: self.conversation_history.clone(),
        };
        
        // Generate response using the model
        let response = self.model_provider.generate_response(input, &context).await?;
...
impl DAGChatCommands {
    ...
    pub async fn analyze_current_dag(&self) -> Result<()> {
        if let Some(dag) = &self.current_dag {
            ...
            let context = ModelContext {
                ...
                system_prompt: Some("You are Jorm AI, specialized in DAG analysis and optimization.".to_string()),
                conversation_history: vec![],
            };
            
            // Generate analysis
            let analysis_prompt = format!("Analyze this DAG for optimization opportunities: {}", dag.name);
            let analysis = self.model_provider.generate_response(&analysis_prompt, &context).await?;
    ...
    pub async fn generate_dag(&self, description: &str) -> Result<Dag> {
        ...
        let _response = self.model_provider.generate_response(&generation_prompt, &context).await?;
    ...
    pub async fn explain_error(&self, error: &str, context: &str) -> Result<()> {
        ...
        let explanation = self.model_provider.generate_response(&explanation_prompt, &model_context).await?;
