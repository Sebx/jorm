use super::{JobResult, OverlapPolicy, ScheduledJob, SchedulerEvent};
...
impl CronScheduler {
    ...
    pub async fn add_job(&self, job: ScheduledJob) -> Result<Uuid> {
        ...
        self.event_sender
            .send(SchedulerEvent::JobScheduled(job_id))
    ...
    pub async fn remove_job(&self, job_id: Uuid) -> Result<bool> {
        ...
        if removed {
            ...
            if let Some(handle) = running_jobs.remove(&job_id) {
                handle.abort();
                self.event_sender
                    .send(SchedulerEvent::JobCancelled(job_id))
    ...
    pub async fn start(&self) -> Result<()> {
        self.event_sender
            .send(SchedulerEvent::SchedulerStarted)
    ...
    pub async fn stop(&self) -> Result<()> {
        ...
        self.event_sender
            .send(SchedulerEvent::SchedulerStopped)
    ...
    async fn execute_job_internal(
        ...
        // Parse the DAG file
        let dag = parse_dag_file(&job.dag_file)
            .await
        ...
        let output = format!(
            ...
            result
                .task_results
                .iter()
                .filter(|(_, r)| r.status == crate::executor::TaskStatus::Success)
                .count(),
            result
                .task_results
                .iter()
                .filter(|(_, r)| r.status == crate::executor::TaskStatus::Failed)
                .count()
        );
