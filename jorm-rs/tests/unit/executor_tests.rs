...
use jorm_rs::executor::{
    ExecutorConfig, NativeExecutor, ExecutionStatus
};
use jorm_rs::parser::{Dag, Task as DagTask, TaskConfig as DagTaskConfig, Dependency};
...
fn create_test_dag() -> Dag {
    ...
    tasks.insert("task1".to_string(), DagTask {
    ...
    tasks.insert("task2".to_string(), DagTask {
...
fn create_python_test_dag() -> Dag {
    let mut tasks = HashMap::new();
    tasks.insert("python_task".to_string(), DagTask {
...
mod task_execution_tests {
    ...
    async fn test_python_task_execution() {
        ...
        assert!(matches!(execution_result.status, ExecutionStatus::Success | ExecutionStatus::Failed));
    }
...
mod error_handling_tests {
    ...
    async fn test_invalid_task_type() {
        ...
        let mut tasks = HashMap::new();
        tasks.insert("invalid_task".to_string(), DagTask {
    ...
    async fn test_failing_task() {
        ...
        let mut tasks = HashMap::new();
        tasks.insert("failing_task".to_string(), DagTask {
...
mod performance_tests {
    ...
    async fn test_execution_performance() {
        ...
        assert!(result.is_ok());
        assert!(duration.as_secs() < 10, "Execution should complete within 10 seconds");
    }
