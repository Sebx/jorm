...
fn test_interactive_mode_startup() {
    ...
    assert!(
        stdout.contains("interactive") || stdout.contains("jorm") || stdout.contains(">"),
...
fn test_interactive_mode_quit() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive mode should exit with quit"
    );
}
...
fn test_interactive_version_command() {
    ...
    assert!(
        stdout.contains("Jorm-RS") || stdout.contains("jorm-rs") || stdout.contains("version"),
...
fn test_interactive_help_command() {
    ...
    assert!(
        stdout.contains("help") || stdout.contains("command") || stdout.contains("usage"),
...
fn test_interactive_status_command() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive status should not crash"
    );
}
...
- task1"#;
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive DAG validation should not crash"
    );
}
...
- task1"#;
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive DAG execution should not crash"
    );
}
...
- task2 after task1"#;
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive DAG description should not crash"
    );
}
...
- task2 after task1"#;
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive DAG with dependencies should not crash"
    );
}
...
fn test_interactive_model_info() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive model info should not crash"
    );
}
...
- task1"#;
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive analyze should not crash"
    );
}
...
fn test_interactive_generate_command() {
    ...
    let (exit_code, stdout, stderr) =
        tester.run_interactive_command("generate simple data pipeline\nexit\n");

    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive generate should not crash"
    );
}
...
fn test_interactive_ai_fallback() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive AI fallback should not crash"
    );
}
...
- task1"#;
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive schedule should not crash"
    );
}
...
fn test_interactive_daemon_command() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive daemon should not crash"
    );
}
...
fn test_interactive_jobs_command() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive jobs should not crash"
    );
}
...
fn test_interactive_trigger_command() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive trigger should not crash"
    );
}
...
fn test_interactive_stop_command() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive stop should not crash"
    );
}
...
fn test_interactive_natural_language_simple() {
    ...
    let (exit_code, stdout, stderr) =
        tester.run_interactive_command("create a simple data pipeline\nexit\n");
...
fn test_interactive_natural_language_complex() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive complex natural language should not crash"
    );
}
...
fn test_interactive_natural_language_questions() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive natural language questions should not crash"
    );
}
...
fn test_interactive_natural_language_help() {
    ...
    let (exit_code, stdout, stderr) =
        tester.run_interactive_command("how do I create a DAG?\nexit\n");

    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive natural language help should not crash"
    );
}
...
fn test_interactive_invalid_command() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive invalid command should not crash"
    );
}
...
fn test_interactive_missing_arguments() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive missing arguments should not crash"
    );
}
...
fn test_interactive_nonexistent_file() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive nonexistent file should not crash"
    );
}
...
fn test_interactive_malformed_input() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive malformed input should not crash"
    );
}
...
fn test_interactive_history_command() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive history should not crash"
    );
}
...
fn test_interactive_multiple_commands() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive multiple commands should not crash"
    );
}
...
- task1"#;
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive command sequence should not crash"
    );
}
...
fn test_interactive_context_preservation() {
    ...
    let (exit_code, stdout, stderr) =
        tester.run_interactive_command("version\nlist\nstatus\nexit\n");

    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive context preservation should not crash"
    );
}
...
- load after transform"#;
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive complex workflow should not crash"
    );
}
...
fn test_interactive_mixed_commands() {
    ...
    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive mixed commands should not crash"
    );
}
...
fn test_interactive_error_recovery() {
    ...
    let (exit_code, stdout, stderr) =
        tester.run_interactive_command("invalid_command\nversion\nhelp\nexit\n");

    assert!(
        exit_code == 0 || exit_code == 1,
        "Interactive error recovery should not crash"
    );
}
