...
fn test_version_command() {
    ...
    assert!(stdout.contains("Jorm-RS") || stdout.contains("jorm-rs"), "Version output should contain 'Jorm-RS' or 'jorm-rs'");
    assert!(stderr.is_empty(), "Version command should not produce stderr");
}
...
fn test_help_command() {
    ...
    assert!(stdout.contains("Usage") || stdout.contains("Commands"), "Help output should contain usage information");
...
fn test_list_command() {
    ...
    assert!(exit_code == 0 || exit_code == 1, "List command should return 0 or 1");
...
- task1"#;
    ...
    assert!(stdout.contains("valid") || stdout.contains("success"), "Validation should report success");
}
...
- hello_task"#;
    ...
    assert!(stdout.contains("Hello from DAG execution"), "DAG output should contain expected text");
}
...
- task2 after task1"#;
    ...
    assert_eq!(exit_code, 0, "DAG with dependencies should execute successfully");
    assert!(stdout.contains("Task 1 executed"), "First task should execute");
    assert!(stdout.contains("Task 2 executed"), "Second task should execute");
}
...
- failing_task"#;
    ...
    assert!(exit_code != 0, "Failing DAG should return non-zero exit code");
}
...
- task2 after task1"#;
    ...
    assert!(stdout.contains("describe_test"), "Description should contain DAG name");
    assert!(stdout.contains("task1") || stdout.contains("task2"), "Description should contain task names");
}
...
fn test_model_info_command() {
    ...
    assert!(exit_code == 0 || exit_code == 1, "Model info should not crash");
}
...
fn test_generate_command() {
    let tester = CLITester::new();
    let (exit_code, stdout, stderr) = tester.run_command(&["generate", "simple data processing pipeline"]);
    
    // Generate might succeed or fail depending on AI setup
    assert!(exit_code == 0 || exit_code == 1, "Generate should not crash");
}
...
- task1"#;
    ...
    let (exit_code, stdout, stderr) = tester.run_command(&["schedule", &dag_path, "--cron", "every 1 minute"]);
    
    // Schedule might succeed or fail depending on scheduler setup
    assert!(exit_code == 0 || exit_code == 1, "Schedule should not crash");
}
...
fn test_help_for_specific_command() {
    ...
    assert!(stdout.contains("run") || stdout.contains("usage"), "Help should contain command information");
}
...
fn test_interactive_help_command() {
    ...
    let (exit_code, stdout, stderr) = tester.run_command_with_input(&["interactive"], "help\nexit\n");
    
    assert!(exit_code == 0 || exit_code == 1, "Interactive help should not crash");
    assert!(stdout.contains("help") || stdout.contains("command"), "Interactive help should show help information");
}
...
fn test_interactive_version_command() {
    ...
    let (exit_code, stdout, stderr) = tester.run_command_with_input(&["interactive"], "version\nexit\n");
    
    assert!(exit_code == 0 || exit_code == 1, "Interactive version should not crash");
    assert!(stdout.contains("Jorm-RS") || stdout.contains("jorm-rs"), "Interactive version should show version");
}
...
fn test_interactive_list_command() {
    ...
    let (exit_code, stdout, stderr) = tester.run_command_with_input(&["interactive"], "list\nexit\n");
...
- task1"#;
    ...
    assert!(exit_code == 0 || exit_code == 1, "Interactive DAG operations should not crash");
}
...
fn test_interactive_ai_features() {
    ...
    let (exit_code, stdout, stderr) = tester.run_command_with_input(&["interactive"], "analyze\nexit\n");
    
    assert!(exit_code == 0 || exit_code == 1, "Interactive AI features should not crash");
}
...
fn test_interactive_scheduler_operations() {
    ...
    let (exit_code, stdout, stderr) = tester.run_command_with_input(&["interactive"], "jobs\nexit\n");
    
    assert!(exit_code == 0 || exit_code == 1, "Interactive scheduler operations should not crash");
}
...
fn test_interactive_natural_language() {
    ...
    let (exit_code, stdout, stderr) = tester.run_command_with_input(&["interactive"], "create a simple data pipeline\nexit\n");
...
fn test_interactive_error_handling() {
    ...
    let (exit_code, stdout, stderr) = tester.run_command_with_input(&["interactive"], "invalid_command\nexit\n");
    
    assert!(exit_code == 0 || exit_code == 1, "Interactive error handling should not crash");
}
...
fn test_interactive_features() {
    ...
    let (exit_code, stdout, stderr) = tester.run_command_with_input(&["interactive"], "history\nexit\n");
    
    assert!(exit_code == 0 || exit_code == 1, "Interactive features should not crash");
}
...
fn test_interactive_clear_command() {
    ...
    let (exit_code, stdout, stderr) = tester.run_command_with_input(&["interactive"], "clear\nexit\n");
