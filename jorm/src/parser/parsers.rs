...
pub fn parse_yaml(content: &str) -> Result<Dag> {
    ...
    if let Some(dependencies_data) = yaml_data.get("dependencies").and_then(|v| v.as_sequence()) {
        for dep_item in dependencies_data {
            // Support two forms:
            // - String entries like "task2 after task1"
            // - Mapping entries like { task: task2, depends_on: task1 }
            if let Some(dep_str) = dep_item.as_str() {
                if let Some((task, depends_on)) = parse_dependency_txt(dep_str) {
                    dag.add_dependency(task, depends_on);
                }
            } else if let Some(mapping) = dep_item.as_mapping() {
                let mut task_name: Option<String> = None;
                let mut depends_on: Option<String> = None;
                for (k, v) in mapping {
                    if let Some(kstr) = k.as_str() {
                        match kstr {
                            "task" => task_name = v.as_str().map(|s| s.to_string()),
                            "depends_on" => depends_on = v.as_str().map(|s| s.to_string()),
                            _ => {}
                        }
                    }
                }
                if let (Some(t), Some(d)) = (task_name, depends_on) {
                    dag.add_dependency(t, d);
                }
            }
        }
    }
