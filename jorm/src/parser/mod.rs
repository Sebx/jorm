...
pub async fn parse_dag_file(file_path: &str) -> Result<Dag> {
    ...
    // First, if the content is valid YAML mapping at top-level, prefer the
    // YAML parser. This avoids mis-classifying YAML files that contain
    // dash-prefixed sequences in nested sections (e.g. dependencies).
    let mut dag = if let Ok(yaml_value) = serde_yaml::from_str::<serde_yaml::Value>(&content) {
        ...
        // If it's not valid YAML, use the heuristic-based selection
        if content.contains("##") {
            if content.contains("tasks:") {
                parsers::parse_txt(&content)?
            } else {
    ...
    if dag.tasks.is_empty() {
        ...
        // If after trying all parsers there are still no tasks, do NOT treat this
        // as a hard error here. Some callers (for example tests or manual triggers)
        // create minimal DAG files that only contain a `dag:` name and no tasks.
        // Return the parsed DAG as-is and let higher-level logic decide whether
        // an empty-task DAG is actionable.
    }
...
fn has_cycle(dag: &Dag) -> bool {
    ...
    for dep in &dag.dependencies {
        // Only add edges where the base (depends_on) task exists in the graph.
        // This avoids panics when validate_dag is called on DAGs with missing
        // tasks; validate_dag will surface the missing-task errors.
        if graph.contains_key(&dep.depends_on) {
            if let Some(vec) = graph.get_mut(&dep.depends_on) {
                vec.push(dep.task.clone());
            }
        }
        // If depends_on is missing, skip the edge; validate_dag already reports the
        // missing dependency and we don't want cycle detection to panic.
    }
