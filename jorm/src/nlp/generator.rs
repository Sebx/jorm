...
impl NlpProcessor {
    ...
    fn parse_multiple_tasks(&self, description: &str) -> Result<Option<Vec<ParsedTask>>, JormError> {
        // Pattern for "N bash/shell tasks that do X"
        let multiple_pattern = Regex::new(r"(?i)(\d+)\s+(?:bash|shell)\s+task(?:s)?\s+that\s+(.+)").unwrap();
        ...
        let workflow_pattern = Regex::new(r"(?i)(?:simple\s+)?workflow\s+with\s+(\d+)\s+(?:bash|shell)\s+task").unwrap();
    ...
    fn split_into_sentences(&self, description: &str) -> Vec<String> {
        ...
        let sentence_regex = Regex::new(r"[,;]\s*|\s+(?:then|and|after|afterwards)\s+|\n+").unwrap();
    ...
    fn parse_sentence(&self, sentence: &str, index: usize) -> Result<Option<ParsedTask>, JormError> {
            ...
            if pattern.is_match(sentence) {
                ...
                if sentence.to_lowercase().contains("copy config files") {
                    ...
                    if sentence.to_lowercase().contains("copy") && captures.len() >= 3 {
                        parameters.insert("source".to_string(), captures.get(1).unwrap().as_str().to_string());
                        parameters.insert("destination".to_string(), captures.get(2).unwrap().as_str().to_string());
                        return Ok(Some(ParsedTask {
                            name: task_name,
                            task_type: "file_copy".to_string(),
                            parameters,
                    ...
                    } else if (sentence.to_lowercase().contains("delete") || sentence.to_lowercase().contains("remove")) && captures.len() >= 2 {
                        parameters.insert("path".to_string(), captures.get(1).unwrap().as_str().to_string());
                        return Ok(Some(ParsedTask {
                            name: task_name,
        ...
        for pattern in &self.http_patterns {
            if let Some(captures) = pattern.captures(sentence) {
                ...
                } else if sentence.to_lowercase().contains("notification") || sentence.to_lowercase().contains("notify") {
                    // Generic notification
                    parameters.insert("url".to_string(), "https://hooks.slack.com/webhook".to_string());
                    parameters.insert("body".to_string(), r#"{"text": "Task completed successfully"}"#.to_string());
                }
        ...
        for pattern in &self.rust_patterns {
            if pattern.is_match(sentence) {
                ...
                let command = if sentence.to_lowercase().contains("build") {
                    "cargo build"
                } else if sentence.to_lowercase().contains("test") || sentence.to_lowercase().contains("run tests") {
                    "cargo test"
                } else if sentence.to_lowercase().contains("run") && !sentence.to_lowercase().contains("tests") {
                    "cargo run"
                } else {
                    "cargo test"
                };
        ...
        for pattern in &self.jorm_patterns {
            if let Some(captures) = pattern.captures(sentence) {
                ...
                } else if sentence.to_lowercase().contains("create") || sentence.to_lowercase().contains("generate") {
                    // jorm generate
                    parameters.insert("command".to_string(), "generate".to_string());
                    parameters.insert("description".to_string(), "Build and test workflow".to_string());
                ...
                } else if sentence.to_lowercase().contains("run") || sentence.to_lowercase().contains("execute") {
                ...
                if sentence.to_lowercase().contains("create") || sentence.to_lowercase().contains("generate") {
                    parameters.insert("command".to_string(), "generate".to_string());
    ...
    fn infer_dependencies(&self, tasks: &mut Vec<ParsedTask>, sentences: &[String]) {
        ...
        for (i, sentence) in sentences.iter().enumerate() {
            if i > 0 && i < tasks.len() {
                ...
                for pattern in &self.dependency_patterns {
                    if pattern.is_match(sentence) {
                        if sentence.to_lowercase().contains("then") || sentence.to_lowercase().contains("after") {
        ...
        for task in tasks {
            ...
            if task.task_type == "jorm" {
                ...
                if let Some(dag_file) = task.parameters.get("dag_file") {
                    ...
                    dag_content.push_str(&format!("    args: [\"--description\", \"{}\"]\n", description));
                }
            } else {
        ...
        for edit in edits {
            match edit {
                DagEdit::RenameTask { old_name, new_name } => {
                    modified_content = modified_content.replace(&format!("task {}", old_name), &format!("task {}", new_name));
                    modified_content = modified_content.replace(&old_name, &new_name);
                },
                DagEdit::ModifyParameter { task_name, parameter, new_value } => {
                    // Simple regex-based parameter modification
                    let pattern = format!(r#"(task {}\s*\{{[^}}]*){}: "[^"]*""#, task_name, parameter);
                    let replacement = format!(r#"$1{}: "{}""#, parameter, new_value);
                    if let Ok(re) = Regex::new(&pattern) {
                        modified_content = re.replace(&modified_content, replacement.as_str()).to_string();
                    }
                },
                DagEdit::AddDependency { task_name, dependency } => {
                    ...
                    if let Ok(re) = Regex::new(&task_pattern) {
                        if modified_content.contains(&format!("task {}", task_name)) && modified_content.contains("depends_on:") {
                            // Add to existing depends_on
                            let depends_pattern = format!(r"(task {}\s*\{{[^}}]*depends_on: \[[^\]]*)", task_name);
                            if let Ok(depends_re) = Regex::new(&depends_pattern) {
                                let replacement = format!("$1, {}", dependency);
                                modified_content = depends_re.replace(&modified_content, replacement.as_str()).to_string();
                            }
                        } else {
...
#[derive(Debug, Clone)]
pub enum DagEdit {
    RenameTask { old_name: String, new_name: String },
    ModifyParameter { task_name: String, parameter: String, new_value: String },
    AddDependency { task_name: String, dependency: String },
}
