...
impl Schedule {
    pub fn new(id: String, cron_expression: String, dag_file: String) -> Result<Self, ScheduleError> {
        ...
        while self.running {
            ...
            for schedule_id in due_schedules {
                if let Some(schedule) = self.schedules.get_mut(&schedule_id) {
                    println!("Executing scheduled DAG: {} ({})", schedule.dag_file, schedule.id);
                    ...
                    match result {
                        Ok(execution_result) => {
                            println!("Scheduled execution completed successfully: {}", schedule.id);
                            if !execution_result.success {
                                eprintln!("Scheduled DAG execution had failures: {}", execution_result.message);
                            }
                        }
                        Err(e) => {
                    ...
                    if let Err(e) = schedule.update_next_execution() {
                        eprintln!("Failed to update next execution time for {}: {}", schedule.id, e);
                    } else if let Some(next) = schedule.next_execution {
                        println!("Next execution for {} scheduled at: {}", schedule.id, next.with_timezone(&Local));
                    }
                }
            }
...
mod tests {
    ...
    async fn test_scheduler_add_remove_schedule() -> Result<(), Box<dyn std::error::Error>> {
        ...
        writeln!(temp_file, "task test {{ type: shell, command: \"echo hello\" }}")?;
