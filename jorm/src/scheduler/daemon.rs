...
use crate::scheduler::cron::{Scheduler, Schedule, ScheduleError};
...
impl Daemon {
    ...
    pub async fn start(&mut self) -> Result<(), DaemonError> {
        ...
        let state = DaemonState {
            ...
            schedules_file: self.schedules_file.as_ref().map(|p| p.to_string_lossy().to_string()),
    ...
    async fn load_schedules(&mut self, schedules_file: &PathBuf) -> Result<(), DaemonError> {
        ...
        println!("Loaded {} schedules from {}", self.scheduler.list_schedules().len(), schedules_file.display());
        Ok(())
    }
