...
impl ShellExecutor {
    ...
    pub async fn execute(
        ...
        match task_type {
            TaskType::Shell {
                ...
                self.execute_shell_command(task_name, command, working_dir.as_deref())
                    .await
            }
            _ => Err(JormError::ExecutionError(format!(
                "Shell executor cannot handle task type: {:?}",
                task_type
            ))),
        }
    }
    ...
    async fn execute_shell_command(
        ...
        if command.trim().is_empty() {
            return Err(JormError::ExecutionError("Empty shell command".to_string()));
        }
