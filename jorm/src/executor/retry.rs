...
impl<T: TaskExecutor> TaskExecutor for RetryExecutor<T> {
    async fn execute(&self, task: &Task, context: &ExecutionContext) -> Result<TaskResult, ExecutorError> {
        ...
        // All retries exhausted - return a new error
        Err(ExecutorError::RetryLimitExceeded {
    ...
    fn clone_box(&self) -> Box<dyn TaskExecutor> {
        // For RetryExecutor, we need to return the inner executor's clone
        // wrapped in a new RetryExecutor, but this creates a type issue.
        // For now, we'll just return the inner executor's clone.
...
pub async fn execute_with_retry<T: TaskExecutor>(
    ...
    for attempt in 0..=retry_config.max_retries {
        match executor.execute(task, context).await {
            ...
            Err(error) => {
                // Check if we should retry this error
                if !retry_config.should_retry(&error, attempt) {
                    return Err(error);
                }
                ...
                if attempt < retry_config.max_retries {
                    ...
                    println!("🔄 Retrying task '{}' in {:?} (attempt {}/{})", 
                            task.name, delay, attempt + 1, retry_config.max_retries);
                    sleep(delay).await;
                }
            }
        }
    ...
    // All retries exhausted - return a retry limit exceeded error
    Err(ExecutorError::RetryLimitExceeded {
        task_id: context.task_id.clone(),
