...
impl JormExecutor {
    ...
    pub async fn execute(&self, task_name: &str, task_type: &TaskType) -> Result<TaskResult, JormError> {
        match task_type {
            TaskType::Jorm { command, args, working_dir } => {
                self.execute_jorm_command(task_name, command, args.as_ref(), working_dir.as_deref()).await
            }
            _ => Err(JormError::ExecutionError(
                format!("Jorm executor cannot handle task type: {:?}", task_type)
            )),
        }
    }
    ...
    async fn execute_jorm_command(
        ...
        if command.trim().is_empty() {
            return Err(JormError::ExecutionError(
                "Empty jorm command".to_string()
            ));
        }

        // Get the current executable path
        let current_exe = std::env::current_exe()
            .map_err(|e| JormError::ExecutionError(format!("Failed to get current executable path: {}", e)))?;
        ...
        match timeout(timeout_duration, cmd.output()).await {
            ...
            Ok(Err(e)) => Err(JormError::ExecutionError(
                format!("Failed to execute jorm command '{}': {}", command, e)
            )),
            Err(_) => Err(JormError::ExecutionError(
                format!("Jorm command '{}' timed out after {} seconds", command, timeout_duration.as_secs())
            )),
        }
    }
}